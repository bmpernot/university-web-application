...
<div class="container">
  <h1 class="mb-4 pb-2">Add a new house for sale</h1>
  ...
  if($_POST){
    ...
    elseif(!$_POST['new']){
      $error = "New or resale not set";
    }
    elseif(!$_POST['modernisation']){
      $error = "In need of modernisation not set";
    }
    elseif(!$_POST['price']){
      $error = "Resale value not set";
    }
    elseif($_POST['price'] < 0){
      $error = "Resale value cannot be less than 0";
    }
    ...
    <?php
      ...
      $AddSale = new Add_Sale($Conn);
      $attempt = $AddSale->createHouse($_POST);

      $houseid = $AddSale->getHouseID($_POST);

      $random = substr(str_shuffle(MD5(microtime())), 0, 10);
      $new_filename = $random.$_FILES['house_thumbnail']['name'];
      if(move_uploaded_file($_FILES['house_thumbnail']['tmp_name'], __DIR__.'/../sale-images/'.$new_filename)){
        $AddSale->addHouseThumbnail($new_filename, $houseid);
      }else {
      ...
      if(move_uploaded_file($_FILES['house_layout']['tmp_name'], __DIR__.'/../sale-images/'.$new_filename)){
        $AddSale->addHouseLayout($new_filename, $houseid);
      }else {
      ...
      foreach ($_FILES['house_image']['tmp_name'] as $house_image_tmp_name) {
        ...
        $random = $random.'.jpg';
        if(move_uploaded_file($house_image_tmp_name, __DIR__.'/../sale-images/'.$random)){
          $AddSale->addHouseImage($random, $houseid);
        } else {
  ...
  <div class="row">
    <div class="col-md-12">
      <form id="reg-sale-form" method="post" action="" enctype="multipart/form-data">

        <div class="form-group">
          <label for="add_sellerID">Seller</label>
          <select class="form-control" id="add_sellerID" name="sellerID">
        ...
        <div class="form-group">
          <label for="add_new">New or Resale</label>
          <select class="form-control" id="add_new" name="new">
            <option value="none" selected disabled hidden>Select an option</option>
            <option value="true">New</option>
            <option value="false">Resale</option>
          </select>
        </div>

        <div class="form-group">
          <label for="add_modernisation">In need of modernisation</label>
          <select class="form-control" id="add_modernisation" name="modernisation">
            <option value="none" selected disabled hidden>Select an option</option>
            <option value="true">Yes</option>
            <option value="false">No</option>
          </select>
        ...
        <div class="form-group">
          <label for="add_price">Resale Value (£)</label>

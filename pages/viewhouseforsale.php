...
$houses = $House->getHousesForSale();
?>
<div class="container">
  <h1 class="mb-4 pb-2">Houses for Sale</h1>
  <?php
  ...
  <?php
    ...
    if (!$_POST['new_or_resale']) {
      $_POST['new_or_resale'] = "none";
    }
    if (!$_POST['modernisation']) {
      $_POST['modernisation'] = "none";
    }
    ...
    $houses = $House->getHousesForSaleWithFiler($_POST);
  }
  ?>
  <div class="row">
    <div class="col-md-3">
    <form id="filters" method="post" action="">
      ...
      <div class="form-group">
        <label for="new_or_resale">New or Resale:</label>
        <select class="form-control" id="new_or_resale" name="new_or_resale">
          <option value="none" selected disabled hidden>Select an option</option>
          <option value=<?php echo true;?>>New</option>
          <option value=<?php echo false;?>>Resale</option>
          <option value="none">Do not mind</option>
        </select>
      </div>

      <div class="form-group">
        <label for="modernisation">Property in need of modernisation:</label>
        <select class="form-control" id="modernisation" name="modernisation">
          <option value="none" selected disabled hidden>Select an option</option>
          <option value=<?php echo true;?>>Yes</option>
          <option value=<?php echo false;?>>No</option>
          <option value="none">Do not mind</option>
      ...
      <div class="row">
        ...
        <div class="col-md-4">
          <div class="house-card">
            <div class="house-card-image" style="background-image: url('./sale-images/<?php echo $house['housethumbnail']; ?>')">
              <a href="index.php?p=houseforsale&id=<?php echo $house['houseforsaleID']; ?>"></a>
            </div>
            <a href="index.php?p=houseforsale&id=<?php echo $house['houseforsaleID']; ?>"><p>
